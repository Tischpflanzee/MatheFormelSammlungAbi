...
Gegeben sei: $ g(x) = x^2 - \textbf{6}x + 11 \Rightarrow \mathbf{-9}\ da \  (\frac{\textbf{6}}{2})^2 $\\
$\Rightarrow x^2 + 6x + \textbf{9 - 9} + 11 $\\
$\Rightarrow (x+3)^2 -9 +11 $\\
$\Rightarrow (x+3)^2 + 2 $

\subsection{Scheitelpunkt und ein weiterer Punkt}
$f(x)=a \cdot (x-x_s)^2 +y_s $\\
x\textsubscript{s} und y\textsubscript{s} = Scheitelpunkt und für x und f(x) kann ein Punkt eingesetz werden!
